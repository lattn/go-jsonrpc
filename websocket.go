...
func (c *wsConn) nextWriter(cb func(io.Writer)) {
	...
	if err != nil {
		slog.Error("websocket NextWriter failed", "error", err)
		cb(io.Discard)
		return
	}
	...
	if err := wcl.Close(); err != nil {
		slog.Error("websocket writer close failed", "error", err)
		return
	}
}
...
func (c *wsConn) handleWsConn(ctx context.Context) {
	...
	for {
		...
		case req := <-c.requests:
			...
			if serr != nil {
				slog.Error("sendRequest failed", "method", req.req.Method, "id", req.req.ID, "error", serr)
